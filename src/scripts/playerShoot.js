...
export function shootBullet(x, y) {
    bullets.forEach(bullet => {
        ...
        enemys.forEach((enemy) => { collisionDetect(enemy, bullet.bottom, bullet.left) })
...
function collisionDetect(enemy, bottom, left) {
    if ((bottom >= enemy.bottom) && (bottom <= (enemy.bottom + 19)) &&
    ((left + 40) >= enemy.left) && (left <= (enemy.left + 100)))
    {
        killEnemy(enemy)
        console.log("hit")
    }
